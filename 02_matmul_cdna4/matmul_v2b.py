    ...
    def kernel(
        ...
        for iter_k in range(K // BLOCK_K):
            ...
            for k in fx.range_constexpr(BLOCK_K // MFMA_K):
                ...
                # scheduling barrier
                # issue all B loads before issuing A loads + MFMA
                # this DOES NOT prevent interleaving B loads with previous MFMA
                rocdl.sched_barrier(0)

                # keep 4 A fragments live at a time
                num_m_iters = WAVE_M // MFMA_M
                assert num_m_iters % 4 == 0
                for i in fx.range_constexpr(num_m_iters // 4):
                    ...
                    # see https://llvm.org/docs/AMDGPUUsage.html llvm.amdgcn.sched.group.barrier for more information
                    rocdl.sched_dsrd(4)
                    rocdl.sched_mfma(4 * (WAVE_N // MFMA_N))
...
_matmul_v2b = build_matmul_v2b()


def matmul_v2b(A: Tensor, B: Tensor):
    M, K = A.shape
    _, N = B.shape
    C = A.new_empty(M, N)
    _matmul_v2b(A, B, C, M, N, K, torch.cuda.current_stream(A.device))
    return C
