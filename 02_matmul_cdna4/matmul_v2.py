...
def build_matmul_v2():
    ...
    def kernel(
        ...
        for iter_k in range(K // BLOCK_K):
            ...
            for k in fx.range_constexpr(BLOCK_K // MFMA_K):
                ...
                rA = [
                    (lds.a.ptr + ((off_am + m * MFMA_M) * BLOCK_K + off_k)).load(T.bf16x8)
                ...
                for m in fx.range_constexpr(WAVE_M // MFMA_M):
                    for n in fx.range_constexpr(WAVE_N // MFMA_N):
                        ...
                        acc[m][n] = rocdl.mfma_f32_16x16x32_bf16(T.f32x4, [rB[n], rA[m], acc[m][n]])
    ...
    def launch(
        ...
        kernel(A, B, C, M, N, K).launch(
            grid=(fx.ceil_div(N, BLOCK_N), M // BLOCK_M, 1),
...
_matmul_v2 = build_matmul_v2()


def matmul_v2(A: Tensor, B: Tensor):
    M, K = A.shape
    _, N = B.shape
    C = A.new_empty(M, N)
    _matmul_v2(A, B, C, M, N, K, torch.cuda.current_stream(A.device))
    return C
