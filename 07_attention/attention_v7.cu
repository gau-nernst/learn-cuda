...
inline __device__ void load_global_to_shared_mem(const nv_bfloat16 *src, uint32_t dst, int ld_src, int tid, bool is_debug = false) {
  ...
  for (int iter_id = 0; iter_id < num_iters; ++iter_id) {
    ...
    const uint32_t new_dst = swizzle<smem_cols * sizeof(nv_bfloat16)>(dst +  (cur_thread_row * smem_cols + cur_thread_col) * sizeof(nv_bfloat16));
    const uint32_t no_s_ds = dst + (cur_thread_row * smem_cols + cur_thread_col) * sizeof(nv_bfloat16);
    // if (is_debug && iter_id == 0) {
    //   printf("tid, %d, iter_id, %d, cur_thread_base_id, %d, cur_thread_row, %d, cur_thread_col, %d no_s_ds, %d, new_dst, %d smem_rows, %d, smem_cols, %d \n", tid, iter_id,
    //          cur_thread_base_id, cur_thread_row, cur_thread_col, no_s_ds, new_dst, smem_rows, smem_cols);
    // }
    // dst postion
    ...
    asm volatile("cp.async.cg.shared.global [%0], [%1], 16;" ::"r"(new_dst), "l"(src_addr));
  }
}

// https://developer.download.nvidia.com/video/gputechconf/gtc/2020/presentations/s21745-developing-cuda-kernels-to-push-tensor-cores-to-the-absolute-limit-on-nvidia-a100.pdf
inline __device__ void load_matrix_x4(uint32_t dst_regs[4], uint32_t src_addr) {
  asm volatile("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];"
               : "=r"(dst_regs[0]), "=r"(dst_regs[1]), "=r"(dst_regs[2]), "=r"(dst_regs[3])
               : "r"(src_addr));
}
inline __device__ void load_matrix_x2(uint32_t dst_regs[2], uint32_t src_addr) {
  asm volatile("ldmatrix.sync.aligned.x2.m8n8.shared.b16 {%0, %1}, [%2];"
               : "=r"(dst_regs[0]), "=r"(dst_regs[1])
               : "r"(src_addr));
}
// https://docs.nvidia.com/cuda/parallel-thread-execution/#warp-level-matrix-instructions-ldmatrix
inline __device__ void load_matrix_x2_tran(uint32_t dst_regs[2], uint32_t src_addr) {
...
__launch_bounds__(NUM_WARPS *WARP_SIZE) __global__
    void attention_v7_kernel(const nv_bfloat16 *Q, const nv_bfloat16 *K, const nv_bfloat16 *V, nv_bfloat16 *O,
  ...
  load_global_to_shared_mem<TB_SIZE, BLOCK_Q, DIM>(Q, query_smem_ptr, DIM, tid, true);
  ...
  for (int mma_m_id = 0; mma_m_id < first_mma_num_m; ++mma_m_id) {
    for (int mma_k_id = 0; mma_k_id < first_mma_num_k; ++mma_k_id) {
      ...
      const uint32_t cur_lane_query_smem_addr_swizzled = swizzle<DIM * sizeof(nv_bfloat16)>(cur_lane_query_smem_addr);
      load_matrix_x4(cur_mma_query_regs, cur_lane_query_smem_addr_swizzled);
      __syncthreads();
    ...
    //   ) {
    //     printf("v66666 mma_k_id, %d, tid, %d, cur_lane_row, %d, cur_lane_col, %d \n", mma_k_id, tid, cur_lane_row, cur_lane_col);
  ...
  for (int kv_iter = 0; kv_iter < num_kv_iters; kv_iter++) {
    ...
    for (int mma_n_id = 0; mma_n_id < first_mma_num_n; mma_n_id++) {
      for (int mma_k_id = 0; mma_k_id < first_mma_num_k; mma_k_id++) {
        ...
        const uint32_t cur_lane_key_smem_addr_swizzled = swizzle<DIM * sizeof(nv_bfloat16)>(cur_lane_key_smem_addr);
        load_matrix_x2(key_regs[mma_n_id][mma_k_id], cur_lane_key_smem_addr_swizzled);
    ...
    */
    ...
    //   }
    // ** Load value from global mem to shared mem
    load_global_to_shared_mem<TB_SIZE, BLOCK_KV, DIM>(V, value_smem_ptr, DIM, tid, true);
    asm volatile("cp.async.commit_group;");
    asm volatile("cp.async.wait_all;");
    __syncthreads();
    ...
    for (int second_mma_k_id = 0; second_mma_k_id < second_mma_num_k; second_mma_k_id++) {
      for (int second_mma_n_id = 0; second_mma_n_id < second_mma_num_n; second_mma_n_id++) {
        ...
        // printf("start load value to register, kv_iter: %d, row: %d, colx: %d \n", kv_iter, cur_lane_row,
        // cur_lane_col);
        const uint32_t cur_lane_value_gmem_addr_swizzled =
            swizzle<DIM * sizeof(nv_bfloat16)>(cur_lane_value_gmem_addr);
        load_matrix_x2_tran(value_regs[second_mma_k_id][second_mma_n_id], cur_lane_value_gmem_addr_swizzled);
        // __syncthreads();
    ...
    for (int second_mma_m_id = 0; second_mma_m_id < second_mma_num_m; second_mma_m_id++) {
      for (int second_mma_n_id = 0; second_mma_n_id < second_mma_num_n; second_mma_n_id++) {
        for (int second_mma_k_id = 0; second_mma_k_id < second_mma_num_k; second_mma_k_id++) {
          ...
          //            output_regs[second_mma_m_id][second_mma_n_id][0],
          //            output_regs[second_mma_m_id][second_mma_n_id][1],
          //            output_regs[second_mma_m_id][second_mma_n_id][2],
          //            output_regs[second_mma_m_id][second_mma_n_id][3]);
          //   } // end of show
        }
      }
      //   __syncthreads();
...
void attention_v7(const nv_bfloat16 *Q, // [bs, len_q, DIM]
  ...
  attention_v7_kernel<BLOCK_Q, BLOCK_KV, DIM, NUM_WARPS><<<grid, block>>>(Q, K, V, O, bs, len_q, len_kv);
