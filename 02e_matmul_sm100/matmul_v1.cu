...
void matmul_v1_launch(
  ...
  // input layout for tcgen05: contiguous blocks of (MMA_M, 8)
  if constexpr (TMAP_3D) {
    // 3D tensormap (WIDTH / 8, HEIGHT, 8) : (8, WIDTH, 1)
    auto init_tmap_AB = [&](CUtensorMap *tmap, const nv_bfloat16 *ptr, uint64_t global_height, uint32_t shared_height) {
      constexpr uint32_t rank = 3;
      uint64_t globalDim[rank]       = {8, global_height, (uint64_t)K / 8};
      uint64_t globalStrides[rank-1] = {(uint64_t)K * sizeof(nv_bfloat16), 16};  // in bytes
      uint32_t boxDim[rank]          = {8, shared_height, (uint32_t)BLOCK_K / 8};
...
void matmul_v1a(
  ...
  matmul_v1_launch<256, 256, false>(A_ptr, B_ptr, C_ptr, M, N, K);
}
...
void matmul_v1b(
  ...
  matmul_v1_launch<256, 256, true>(A_ptr, B_ptr, C_ptr, M, N, K);
}
